class EndLevel extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("wizard", "image/mad scientist.png",
        {frameWidth: 32, frameHeight: 32});
    this.load.image("cake", "image/the cake is a lie.png");
  ...
  create() {
    ...
    this.wizardText = this.add.text(120, 720, "Thanks for playing!\nHave some cake.");

    // Cake
    this.cake = this.physics.add.image(360, 765, "cake");
  }
  ...
  update() {
    ...
    if (Math.pow(this.player.sprite.x - this.wizard.x, 2) + Math.pow(this.player.sprite.y - this.wizard.y, 2) <= 69 ** 2) {
